...
class TAL:
    ...
    def __call__(self, outputs, targets):
        ...
        mask = target_scores.gt(0).to(torch.bool)[fg_mask]
        ...
        # cls loss
        loss_cls = self.bce(pred_scores, target_scores.float())
        ...
        if fg_mask.sum().clamp(min=1.0):
            ...
            target_lt_rb = torch.cat((anchor_points.unsqueeze(0) - a, b - anchor_points.unsqueeze(0)), -1)
    ...
    def assign(self, pred_scores, pred_bboxes, true_labels, true_bboxes, true_mask, anchors, stride_tensor):
        ...
        if self.num_max_boxes == 0:
            ...
            target_scores = torch.zeros_like(pred_scores).to(device)  # 保持维度一致 [B, 1029, nclass]
            target_bboxes = torch.zeros_like(pred_bboxes).to(device)
            fg_mask = torch.zeros_like(pred_scores[..., 0], dtype=torch.bool).to(device)
...
class SimOTA:
    ...
    def __call__(self, outputs, targets):
        ...
        # cls loss
        pos_scores = pred_scores[fg_mask][mask].sigmoid().clamp(min=self.eps, max=1. - self.eps)
    ...
    def assign(self, pred_scores, pred_bboxes, true_labels, true_bboxes, true_mask, anchors, stride_tensor):
        ...
        # [B, 1029, nbox, nclass]
        pred_scores_loss = pred_scores.unsqueeze(-2).repeat([1, 1, overlaps.shape[1], 1]) * (
            overlaps.permute(0, 2, 1).contiguous().unsqueeze(-1))
        ...
        scores_loss = ((F.binary_cross_entropy(pred_scores_loss, true_scores, reduction='none')).sum(-1)).permute(0, 2,
                                                                                                                  1).contiguous()
